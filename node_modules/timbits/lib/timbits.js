(function() {
  ...
  this.serve = function(options) {
    ...
    this.server.get('/timbits/json', function(req, res) {
      return res.json(_this.box);
    });
    this.server.all('/timbits/help', function(req, res) {
      return res.send(ck.render(views.help, {
        box: _this.box
      }));
    });
    this.server.all('/timbits/test', function(req, res) {
      ...
      if (pending) {
        ...
        for (timbit in _this.box) {
          _results.push(request({
            ...
            master += body;
            if (!--pending) return res.end(master);
          }));
        }
        return _results;
      } else {
  ...
  this.add = function(name, timbit) {
    ...
    return this.server.all("/" + name + "/:view?", function(req, res) {
      var attr, context, k, key, v, value, _base, _ref, _ref2, _ref3;
      try {
        context = {};
        _ref = req.query;
        for (k in _ref) {
          v = _ref[k];
          if ((v != null) && v !== '') context[k.toLowerCase()] = v;
        }
        context.name = timbit.name;
        context.view = "" + timbit.viewBase + "/" + ((_ref2 = (_base = req.params).view) != null ? _ref2 : _base.view = 'default');
        context.maxAge = timbit.maxAge;
        _ref3 = timbit.params;
        for (k in _ref3) {
          attr = _ref3[k];
          key = k.toLowerCase();
          if (context[key] == null) context[key] = attr["default"];
          value = context[key];
          if (value != null) {
          ...
          if (attr.required && !value) throw "" + key + " is a required parameter";
